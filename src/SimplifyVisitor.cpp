...
auto SimplifyVisitor::TypedVisit(const Add<>& add) -> RetT
{
    ...
    if (options.simplifyFractions) {
        if (auto realCase = RecursiveCast<Add<Real, Divide<Real>>>(simplifiedAdd); realCase != nullptr) {
            ...
            return gsl_lite::not_null { std::make_unique<Divide<Real>>(Real { frac.GetLeastSigOp().GetValue() * firstReal.GetValue() + frac.GetMostSigOp().GetValue() }, frac.GetLeastSigOp()) };
...
auto SimplifyVisitor::TypedVisit(const Divide<>& divide) -> RetT
{
        ...
        if (options.simplifyFractions) {
            ...
            if ((std::abs(dividend_long - dividend.GetValue()) <= EPSILON)
                ...
                if (gcf != 1) {
                    ...
                    return gsl_lite::not_null { std::make_unique<Divide<Real>>(Real { static_cast<double>(dividend_long / gcf) }, Real { static_cast<double>(divisor_long / gcf) }) };
                } else {
                    // ex. 3/1 or 3/3
                    ...
                    if (divisor_long != 1 && divisor_long != dividend_long
                        ...
                        return gsl_lite::not_null { std::make_unique<Divide<Real>>(dividend, divisor) };
                    }
                }
            } else {
    ...
    if (auto realCase = RecursiveCast<Divide<Divide<Real>>>(simplifiedDivide); realCase != nullptr) {
        ...
        return gsl_lite::not_null { std::make_unique<Multiply<Divide<Real>>>(arg1, reciprocal) };
    }
...
auto SimplifyVisitor::TypedVisit(const Exponent<>& exponent) -> RetT
{
    static auto match_cast = MatchCast<Expression>()
                                 ...
                                 .Case(
                                     ...
                                     [](const Exponent<Divide<Real>, Real>& ImgCase, const void*) -> std::expected<gsl_lite::not_null<std::unique_ptr<Expression>>, std::string_view> {
                                         Divide div {
                                             Real {
                                        ...
                                        return gsl_lite::make_not_null(div.Copy());
                                     })
                                 .Case(
